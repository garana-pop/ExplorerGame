...
public class NarrationSoundController : MonoBehaviour
{
    ...
    private void PlaySoundEffect(string soundKey)
    {
        ...
        if (manager == null)
        {
            Debug.LogError($"{nameof(NarrationSoundController)}: SoundEffectManagerが見つかりません");
            return;
        }
        ...
        if (debugMode)
        {
            Debug.Log($"{nameof(NarrationSoundController)}: 効果音 '{soundKey}' を再生しました (Volume: {soundVolume})");
        }
    }
